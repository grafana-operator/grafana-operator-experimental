...
var _ = Describe("Grafana controller", func() {
	...
	Context("When creating Grafana", func() {
		It("Should create expected resources", func() {
			...
			By("By reaching the last reconciler stage")
			Eventually(func() grafanav1beta1.OperatorStageName {
				err := k8sClient.Get(ctx, grafanaLookupKey, createdGrafana)
				if err != nil {
					return ""
				}
				return createdGrafana.Status.Stage
			}, timeout, interval).Should(Equal(grafanav1beta1.OperatorStageDeployment))
			...
			createdGrafana.Status.Plugins = grafanav1beta1.PluginList{
				grafanav1beta1.GrafanaPlugin{
